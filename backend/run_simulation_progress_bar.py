...
class Simulation:
    ...
    def run(self):
        ...
        self._run_job()
        self._wait_for_job_completion(mdb.jobs[self.modelName])
    ...
    def _wait_for_job_completion(self, job_name, total_target_time):
        sta_file_path = "{}.sta".format(job_name)        
        self.log("      - Monitoring simulation progress...", self.logFilePath)

        # Variables to throttle logging so it doesn't spam the UI/log file every 0.5s
        last_log_time = start_wall_time
        log_interval_seconds = 5.0 

        while True:
            # 1. Check if file exists yet (Abaqus might take a moment to generate it)
            if not os.path.exists(sta_file_path):
                time.sleep(0.5)
                continue

            # 2. Read the file contents safely
            try:
                with open(sta_file_path, "r", encoding="utf-8") as f:
                    lines = f.readlines()
            except IOError:
                # File might be locked by Abaqus during a write cycle; try again next loop
                time.sleep(0.5)
                continue
            ...
            # 3. Check for analysis completion or failure
            # We check a larger chunk of the end of the file to be safe
            last_few_lines = "".join(lines[-10:]).upper()
            if "COMPLETED SUCCESSFULLY" in last_few_lines or "SUCCESSFULLY COMPLETED" in last_few_lines:
                self.log("      - Job completed successfully.", self.logFilePath)
                break
            elif "ERROR" in last_few_lines or "TERMINATED" in last_few_lines:
                self.log("      - Job FAILED or was TERMINATED. Check .msg or .dat files.", self.logFilePath)
                raise RuntimeError("Abaqus simulation aborted with errors.")

            # 4. Parse the last valid data row to get the current simulation time
            # Based on the Abaqus/Explicit format provided:
            # col 0: INCREMENT | col 1: STEP TIME | col 2: TOTAL TIME | col 3: CPU TIME
            last_data_row = None
            for line in reversed(lines):
                parts = line.split()
                # Check if the row has at least 8 columns and starts with a numeric Increment
                if len(parts) >= 8 and parts[0].isdigit():
                    # Double-check that column 3 is the CPU time containing a colon (e.g., '00:00:00')
                    if ":" in parts[3]:
                        last_data_row = parts
                        break

            # 5. Calculate the rule of 3 for the remaining time
            if last_data_row:
                try:
                    # Extract TOTAL TIME from index 2
                    current_sim_time = float(last_data_row[2])
                    
                    if current_sim_time > 0 and total_target_time > 0:
                        progress_ratio = current_sim_time / total_target_time
                        
                        # Cap progress at 1.0 (100%) to avoid strange UI behavior if Abaqus slightly overshoots
                        progress_ratio = min(progress_ratio, 1.0)
                        
                        elapsed_wall_time = time.time() - start_wall_time
                        
                        # Rule of 3: Estimated Total Time = Elapsed Wall Time / Progress Ratio
                        estimated_total_wall_time = elapsed_wall_time / progress_ratio
                        remaining_wall_time = estimated_total_wall_time - elapsed_wall_time
                        
                        # Log the estimation periodically rather than every 0.5s
                        current_time = time.time()
                        if current_time - last_log_time >= log_interval_seconds:
                            progress_percent = progress_ratio * 100
                            self.log(
                                "      - Progress: {:.1f}% | Estimated remaining time: {:.0f} seconds".format(
                                    progress_percent, remaining_wall_time
                                ), 
                                self.logFilePath
                            )
                    ...
                    # Ignore parsing errors if Abaqus writes a malformed row mid-write
                    pass
